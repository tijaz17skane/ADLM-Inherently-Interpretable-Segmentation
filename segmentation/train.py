...
def train(
    ...
    if not pruned:
        ...
        log('MAIN TRAINING')
        callbacks = [
            EarlyStopping(monitor='val/auroc', patience=early_stopping_patience_main, mode='max')
        ]
    ...
    callbacks = [
        EarlyStopping(monitor='val/auroc', patience=early_stopping_patience_last_layer, mode='max')
    ]
